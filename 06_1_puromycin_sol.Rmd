...
Puromycin %>%
  ...
  geom_point() +
  ## To aid the visualization, we can fit a straight line to the data
  stat_smooth(method = "lm", col = "firebrick", se = FALSE) +
  ylab("Reaction Rate (counts/min)") +
...
```{r}
mod1 <- lm(rate ~ log10(conc), Puromycin)
summary(mod1)
```
...
### Homoskedasticity: equality of the variance
