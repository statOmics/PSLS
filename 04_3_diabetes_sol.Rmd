...
```{r}
diabetes_long %>%
  ## Calculate summarry statistics for the "concentration" variable for each "time"
  group_by(time) %>%
  summarize(
...
Note that we also convertthe variable time in a factor first and relevel it so that the concentration before is plotted first. 
